package com.princethakur.tinylink.services;
...
import com.princethakur.tinylink.models.ClickEvent;
import com.princethakur.tinylink.models.UrlData;
...
public class UrlShortenerService {
    ...
    private String buildShortUrl(String shortCode) {
        ...
        String normalizeBaseUrl = baseUrl.endsWith("/") ? baseUrl.substring(0, baseUrl.length() - 1) : baseUrl;
        return normalizeBaseUrl + "/api/" + shortCode;
    }
    ...
    private String getCacheUrl(String shortCode) {
        try {
    ...
    public void recordClick(String shortCode, String clintIp, String userAgent, String referrer) {
        UrlData urlData = urlMappings.get(shortCode);
        if (urlData != null && urlData.isActive()) {
            ...
            log.debug("Recoder click for short code : {}", shortCode);
        }
    }
    ...
    public Optional<UrlAnalyticsResponse> getUrlAnalytics(String shortCode) {
        ...
        Map<String, Integer> clicksByDay = clicks.stream()
                .collect(Collectors.groupingBy(c -> c.getTimeStamp().toLocalDate().toString(), Collectors.summingInt(e -> 1)));


        List<ClickEvent> recentClicks = clicks.stream()
                .sorted((a, b) -> b.getTimeStamp().compareTo(a.getTimeStamp())).limit(10).toList();
