...
import com.princethakur.tinylink.models.RateLimitData;
...
public class RateLimitService {
    ...
    @Value("${tinylink.rate-limits.requests-per-minute}")
    private int requestsPerMinute;
    @Value("${tinylink.rate-limits.request-per-hour}")
    ...
    public long getTimeUntilReset(String clientIp) {
        ...
        if (data.getHourCount() >= requestsPerHour) {
            LocalDateTime nextHour = data.getHourWindowStart().plusMinutes(1);
